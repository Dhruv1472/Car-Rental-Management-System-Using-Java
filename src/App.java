...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (con != null) {
            ...
            System.out.println("");
            System.out.println(
                    "WANT TO 1. REGISTER OR 2. LOGIN (if you are not registered so please first register)");
            ...
            do {
                ...
                if (music_id != -1) {
                    ...
                    PreparedStatement fetch_musicname_pst = con.prepareStatement(
                            fetch_musicname_sql);
    ...
    static int register() throws Exception {
        ...
        do {
            ...
            do {
                ...
                if (dotcheck != -1) {
                    ...
                    if (ext.equalsIgnoreCase(".com") &&
                            emialaddress.length() - emialaddress.lastIndexOf('&') > 4) {
                        String checkemailsql = "select email from usersdata where email=?";
                        PreparedStatement checkmailpst = con.prepareStatement(
                                checkemailsql);
            ...
            do {
                System.out.print(
                        "  ENTER THE PASSWORD (password contains maximum 8 character): ");
                pass = sc.next();
                if (pass.length() <= 8) {
                    verify = true;
                }
...
class Music {
    ...
    public void setplaylist(Connection con, int user_id) throws Exception {
        ...
        PreparedStatement fetch_playlist_pst = con.prepareStatement(
                fetch_playlist_file);
        fetch_playlist_pst.setInt(1, user_id);
        ResultSet fetch_playlist_rs = fetch_playlist_pst.executeQuery();
        if (fetch_playlist_rs.next()) {
            playlist_name = fetch_playlist_rs.getString("playlist_name");
            playlist_path = fetch_playlist_rs.getString("playlist_path");
    ...
    public void setvar(Connection con) throws Exception {
        ...
        while (fetchrs.next()) {
            ...
            Music_details md = new Music_details(
                    music_id,
                    music_name,
                    music_path,
                    music_size,
    ...
    public void setprevpath() throws Exception {
        int currentplayindex = mf.currentplayindex;
        int prevplayindex = currentplayindex - 1;
        if (prevplayindex - 1 <= 0) {
    ...
    public int showmusics(Connection con) {
        if (total_musics > 0) {
            ...
            for (Map.Entry<Integer, Music_details> entry : md_arr.entrySet()) {
                ...
                if (entry.getKey() == 1) {
                    System.out.println(
                            "     SR.NO     |     MUSIC NAME     |     SIZE     |     DURATION  ");
                }
                System.out.println(
                        "     " +
                                entry.getKey() +
                                "                " +
                                entry.getValue().music_name +
                                "              " +
                                entry.getValue().music_size +
                                "              " +
                                entry.getValue().music_duration);
                System.out.println("");
            }
            ...
            do {
                System.out.println(
                        "FOR PLAY MUSIC ENTER THE MUSIC SR.NO (else enter the -1 for next process)");
                ...
                if (music_choise != -1) {
                    if (music_choise > 0 && music_choise <= total_musics) {
                        ...
                        try {
                            ...
                            do {
                                System.out.println("  A.PREV  B.PAUSE  C.NEXT E.EXIT");
    ...
    public int searchmusic(Connection con) {
        ...
        if (music_count == 1) {
            ...
            System.out.println(
                    "  FOR PLAY ENTER SONG SR.NO (else enter the -1 for next process)");
    ...
    public void createplaylist(Connection con, int user_id) throws Exception {
        ...
        if (!playlist_created) {
            ...
            PreparedStatement insertplaylist_pst = con.prepareStatement(
                    insertplaylist);
            ...
            if (r > 0) {
                ...
                do {
                    System.out.println();
                    System.out.print(
                            "  ENTER THE SR.NO WITH SPACE SEPERATED FOR ADD SONGS INTO " +
                                    playlist_name +
                                    " PLAYLIST: ");
                    sc.nextLine();
                    ...
                    for (int i = 0; i < playlist_add_arr.length; i++) {
                        ...
                        if (checkingnumberformatexception == false) {
                            ...
                            String file_writer = music_id +
                                    " " +
                                    music_playlist_chosie +
                                    " " +
                                    music_path +
                                    "\n";
        ...
        do {
            ...
            if (playlist_choise == 1) {
                for (Music_details play_music : my_playlist) {
                    ...
                    System.out.println("  A].PREV  B].PAUSE  C].NEXT");
                    System.out.print("  CHOISE: ");
                    mf.play_playlist();
                }
                // }
                // if (playlist_choise == 1) {
                // List<Music_details> songsList = new ArrayList<>();
                // songsList.addAll(my_playlist);

                // int currentSongIndex = 0; // Initialize with the first song
                // boolean continuePlayback = true;

                // while (continuePlayback) {
                // Music_details play_music = songsList.get(currentSongIndex);
                // mf.setpath(play_music.music_id, play_music.music_path, true);
                // System.out.println("");
                // System.out.println("CURRENT PLAY: " + play_music.music_name);
                // System.out.println(
                // "A].PREV B} PLAY FULL PLAYLIST C].NEXT D].STOP"
                // );
                ...
                // currentSongIndex = songsList.size() - 1; // Wrap to the last song
                // }
                // mf.play();
                // break;
                // case 'B':
                ...
                // case 'D':
                // mf.pause();
                // continuePlayback = false; // Exit the loop and stop playback
                // break;
                // default:
                    ...
                    for (int i = 0; i < playlist_add_arr.length; i++) {
                        ...
                        if (checkingnumberformatexception == false) {
                            ...
                            if (check_music_id == true) {
                                System.out.println(
                                        "  " +
                                                md_arr.get(music_playlist_chosie).music_name +
                                                " IS ALREDY EXISTS IN YOUR PLAYLIST");
                                System.out.println("");
                            } else {
                                String music_path = md_arr.get(music_playlist_chosie).music_path;
                                String file_writer = music_id +
                ...
                BufferedReader br_remove = new BufferedReader(
                        new FileReader(playlist_path));
                System.out.print(
                        "  ENTER THE SR.NO WITH SPACE SEPERATED WHICH SONG DO YOU WANT TO REMOVE FROM YOUR PLAYLIST: ");
                sc.nextLine();
                String playlist_remove = sc.nextLine();

                String playlist_remove_arr[] = playlist_remove.split(" ");
                String playlist_main_data[] = new String[my_playlist.size() -
                        playlist_remove_arr.length];

                if (playlist_remove_arr.length > my_playlist.size()) {
                    System.out.println("");
                    System.out.println(
                            "  ENTERED TOTAL MUSIC NO IS GREATER THAN AVAILABLE PLAYLIST SONGS...!");
                    ...
                    for (int i = 1; i <= my_playlist.size(); i++) {
                        ...
                        for (int j = 0; j < playlist_remove_arr.length; j++) {
                            ...
                            if (music_index == i) {
                                my_playlist.remove(music_index);
                                add = false;
                                break;
                            }
                        }
...
class MusicFeatures {
    ...
    public void setpath(int music_index, String music_path, boolean isplaylist)
